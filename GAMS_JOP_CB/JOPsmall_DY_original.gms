...
CODE BY: Christopher Keyes, Harvey Cutler (2017)
DATA BY: CSU & NIST Center for Community Resliancy


NOTES:
This was prevously Chris's JOPsm_WS_DY script that has been adjusted to use the
elasticities in Harvey's 'jop;incollapseDYN_BH' script and the framework of Chris's
Shelby model
...
TABLE JOBCR(H, L)
            L1                       L2                         L3
HH1        1.087051848106440        0.000000000000000        0.000000000000000
HH2        0.403799600752105        0.791266871035837        0.000000000000000
HH3        0.373326324111829        0.844126722901611        0.265783987310278
HH4        0.358277931692947        1.158123100213240        1.338315562461390
HH5        0.368222983827644        1.086433760477110        1.442728890156260 ;
...
$ONDELIM
*$INCLUDE C:\Dropbox\Joplin\JOP_SMALL\Run_Files\samJOP5HCT_Y1S.CSV
*$INCLUDE C:\Users\hcutler\Dropbox\joplin 1\samjop5HCTF2.CSV
$INCLUDE C:\Users\Zhisheng\Desktop\GMS\samjopT.CSV
$OFFDELIM

TABLE BB(I,IG) CAPITAL COMP
$ONDELIM
*$INCLUDE C:\Dropbox\Joplin\JOP_SMALL\Run_Files\CAPCOMjop.CSV
$INCLUDE C:\Users\Zhisheng\Desktop\GMS\CAPCOMjop.CSV
$OFFDELIM
...
EXWGEO(CM)     CALC    EXTERNAL WAGE commuting out
EXWGEI(L)     CALC    EXTERNAL WAGE commuting in
ECOMI(L)      CALC    ELASTICITY OF LABOR SUPPLY FOR IN COMMUTTERS
ECOMO(CM)      CALC    ELASTICITY OF LABOR SUPPLY FOR OUT COMMUTTERS
...
CMO0(CM)      CALC    REAL    NUMBER COMMUTING OUT
...
DELTA(I)         = DS0(I)/ (PROD(F,FD0(F,I)**ALPHA(F,I)));   display delta;
...
ETAI, ETAL1, ETALB1, ETAIX, ETALB, ETAD, DEPR, ALPHA,

TAUQ, TAUC, TAUV, TAUN, TAUG, TAUX, TAUF, TAUFX, TAUH,

TAUFH, TAUFF, TAUFL, TAUFK, TAXS, TAXS1,

HH0, HW0, HN0, CMI0, CMO0, JOBCOR, FD0, KS0,
...
CMI.L(L)$(ABS(CMI.L(L))                  LT 1)=0;
CMO.L(CM)$(ABS(CMO.L(CM))                LT 1)=0;
...
CMO.LO(CM)       = CMO.L(CM)     / 1000;   CMO.UP(CM)    = CMO.L(CM)     * 1000;
...
PFEQ(I)..DS(I)           =E= DELTA(I)*PROD(F, (TT(F,I)*FD(F,I))**ALPHA(F,I));
...
XEQ(I).. CX(I)           =E= CX0(I)*((PD(I))/(PW0(I)))**(ETAE(I));
...
LSEQ1(H).. HW(H)/HH(H)   =E= HW0(H)/HH0(H)*((SUM(L, RA(L) / RA0(L))/3)/ (CPI(H) / CPI0(H))
...
YGEQ(GX).. Y(GX)         =E= SUM(I, TAUV(GX,I) * V(I) * P(I) )
                                 ...
                                 + SUM(LJ, TAUFH(GX,LJ) * (Y(LJ)))
                                 + SUM(L, TAUFH(GX,L) * (CMIWAGE(L)*CMI(L)))
...
*LMEQ1.. SUM(H, HW(H)* JOBCOR(H,'L1')) + CMI('L1') =E= SUM(Z, FD('L1',Z)) + SUM(CM1, CMO(CM1));
*LMEQ2.. SUM(H, HW(H)* JOBCOR(H,'L2')) + CMI('L2') =E= SUM(Z, FD('L2',Z)) + SUM(CM2, CMO(CM2));
*LMEQ3.. SUM(H, HW(H)* JOBCOR(H,'L3')) + CMI('L3') =E= SUM(Z, FD('L3',Z)) + SUM(CM3, CMO(CM3));
...
LAMEQ(LA,IG).. LAS(LA,IG)=E= TT(LA,IG)*FD(LA,IG);
...
if(index = 2,
*  KS0(K,'HS1')=  KS0(K,'HS1')*1.2;
*  KS0(K,'HS2')=  KS0(K,'HS2')*1.1;
*  KS0(K,'HS3')=  KS0(K,'HS3')*1.02;
*  KS0(K,'GOODS')=  KS0(K,'GOODS')*1.2;
*  KS0(K,'TRADE')=  KS0(K,'TRADE')*1.04;
*  KS0(K,'OTHER')=  KS0(K,'OTHER')*1.2;
*  delta(I)=delta(I)*.99;
* N0(K,I)=N0(K,I)*1.16;
* N0(K,I)=N0(K,I)*1.5;

*  pw0('other')= pw0('other')* 1.02;

* ks0(k,I)= ks0(k,I)*1.15;
* pw0(I) =  pw0(I)*1.01;
...
TEST1(H)         =  HW0(H)/HH0(H)*((SUM(L, RA.L(L) / RA0(L))/3)/ (CPI.L(H) / CPI0(H))
                    * (SUM((Z,L), FD.L(L,Z))/ (SUM(H1, HW.L(H1)* SUM(L, JOBCOR(H1,L))) + SUM(L,CMI.L(L)))));

TEST2(H)         =  SUM((CM,L), EXWGEO(CM)/RA.L(L))/27
                     * ((SUM(CM, CMO.L(CM))/(SUM(H1, HW.L(H1)* SUM(L,JOBCOR(H1,L))) + SUM(L,CMI.L(L)))))**(ETARA(H));

TEST3(H)         =  (SUM(G, TP(H,G)/ CPI.L(H)) / SUM(G, TP(H,G) / CPI0(H) ))**(ETAPT(H))
                    * ((SUM(GI, PIT0(GI,H)* HH0(H))+ SUM(G, TAUH0(G,H)*HH0(H)))
                    / (SUM(GI, PIT(GI,H)* HH.L(H))+ SUM(G, TAUH(G,H)*HH.L(H))))**(ETAPIT(H));
* Total Households

TEST4(H)            = HH0(H) * NRPG(H);

TEST5(H)            = MI0(H) * ((YD.L(H)/HH.L(H))/(YD0(H)/HH0(H))/(CPI.L(H)/CPI0(H))) ** (ETAYDO(H))
                                         *((HN.L(H)/HH.L(H))/(HN0(H)/HH0(H))) ** (ETAUO(H));

TEST6(H)            =  - MO0(H) *((YD0(H)/HH0(H))/(YD.L(H)/HH.L(H))/(CPI0(H)/CPI.L(H))) ** (ETAYDO(H))
                                         *((HN0(H)/HH0(H))/(HN.L(H)/HH.L(H))) ** (ETAUO(H));

* Gross Income

TEST7(H)           = SUM(L,  A(H,L) * HW.L(H) / SUM(H1, A(H1,L) * HW.L(H1) )
                                 * (Y.L(L) - (CMIWAGE(L)*CMI.L(L))) * ( 1 - SUM(G, TAUFL(G,L))));
TEST8(H)           = SUM(CM, A(H,CM)*CMOWAGE(CM)*CMO.L(CM));
TEST9(H)           = SUM(LA,  A(H,LA) * HW.L(H) / SUM(H1, A(H1,LA) * HW.L(H1)) * (Y.L(LA)+ LNFOR.L(LA))*( 1 - SUM(G, TAUFLA(G,LA) ) ) );
TEST10(H)          = SUM(K,  A(H,K) * HW.L(H) / SUM(H1, A(H1,K) * HW.L(H1)) * (Y.L(K) + KPFOR.L(K)) * ( 1 - SUM(G, TAUFK(G,K) ) ) );

$ontext
=E= SUM(L,  A(H,L) * HW(H) / SUM(H1, A(H1,L) * HW(H1) )
                                 * (Y(L) - (CMIWAGE(L)*CMI(L))) * ( 1 - SUM(G, TAUFL(G,L))))
                                 + SUM(CM, A(H,CM)*CMOWAGE(CM)*CMO(CM))
                                 + SUM(LA,  A(H,LA) * HW(H) / SUM(H1, A(H1,LA) * HW(H1)) * (Y(LA)+ LNFOR(LA))*( 1 - SUM(G, TAUFLA(G,LA) ) ) )
                                 + SUM(K,  A(H,K) * HW(H) / SUM(H1, A(H1,K) * HW(H1)) * (Y(K) + KPFOR(K)) * ( 1 - SUM(G, TAUFK(G,K) ) ) );
...
TP, V0, V.L, Y0, Y.L, YD0, YD.L, DCMI.L, DCMO.L,
DFCG.L, DFFD.L, DY.L, DDS.L, DM.L, DDD.L, DCX.L, DRR.L, DGCP1.L, GCP1.L, dgcp.L,
GCP.L, GCP0, CMI0, CMI.L, CMO0, CMO.L, gvfor0, gvfor.l,
pit, etad, privret, alpha, DK.L, TAXB.L, dch.l, TT, DRR.l, TEST1, TEST2, TEST3, TEST4, TEST5, TEST6, TEST7, TEST8,TEST9, TEST10;
...
$ONTEXT
execute_unload "results.gdx" , fstore,dystore, cpistore, hhstore, dsstore;
