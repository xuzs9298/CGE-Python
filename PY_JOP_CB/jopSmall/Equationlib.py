...
class VarContainer:
  ...
  def get_all_variable_printed(self):
    for i in range(len(self.initialVals)):
      label = self.getLabel(i)
      if len(label) == 1:
        print(label[0]+'='+str(self.initialVals[i])+';')
      elif len(label) == 2:
        print(label[0]+'(\''+label[1]+'\')'+'='+str(self.initialVals[i])+';')
      elif len(label) == 2:
        print(label[0]+'(\''+label[1]+'\',\''+label[2]+'\')'+'='+str(self.initialVals[i])+';')
  ...
  def get(self, name, x = None):
    ...
    if 'nrows' in info and 'ncols' in info:
      ...
      for i in info['rows']:
          for j in info['cols']:
              ret = ret.set_value(i,j,x[self.getIndex(name,row=i,col=j)])
    elif 'nrows' in info and 'ncols' not in info:
      ret = pd.Series(index = info['rows']).fillna(0.0)
      for i in info['rows']:
        ret = ret.set_value(i,x[self.getIndex(name,row=i)])
  ...
  def write(self, filename):
    ...
    with open(filename, 'a') as f:
      for i in range(self.nvars):
        lower = -1e10 if self.LO[i] == None else self.LO[i]
        upper = 1e10 if self.UP[i] == None else self.UP[i]
