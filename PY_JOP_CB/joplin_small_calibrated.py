...
# SAM
SAM = pd.read_csv(os.path.join(filePath, 'sam.csv'), index_col=0)
...
IOUT = pd.read_csv(os.path.join(miscPath, 'IOUT(G1,G1).csv'), index_col=0)
MISC = pd.read_csv(os.path.join(miscPath, 'MISC(Z,new).csv'), index_col=0)
MISCH = pd.read_csv(os.path.join(miscPath, 'MISCH(H,new).csv'), index_col=0)
LANDCAP = pd.read_csv(os.path.join(miscPath, 'LANDCAP(IG,new).csv'), index_col=0)
EMPLOY = pd.read_csv(os.path.join(miscPath, 'EMPLOY(Z,F).csv'), index_col=0)
IGTD = pd.read_csv(os.path.join(miscPath, 'IGTD(G,G1).csv'), index_col=0)
TAUFF = pd.read_csv(os.path.join(filePath,'MiscFile/TAUFF(G,F).csv'), index_col=0)
TPC = pd.read_csv(os.path.join(miscPath, 'TPC(H,G).csv'), index_col=0)
...
LAMBDA.columns = I
for i in I:
    LAMBDA.loc[[i], [i]] = MISC.get_value(i, 'ETAOP')
...
#DELTA.loc[I] = round(DELTA.loc[I],3)
# OTHER DATA
...
for ittr in range(iNum):
  ...
  if ittr == 0: # if it is the first simulation, apply the shock
      ...
      #KS0.loc[K, I] = KS0.loc[K, I]*0.8
      NRPG.loc[H] = NRPG.loc[H] * 1.02
      #PW0.loc[I] = PW0.loc[I]*1.02
      '''
